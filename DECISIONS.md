...
1.0.3 (1) usa IDs demo y existe únicamente para comprobar la corrección en iPhone. No se seleccionará para la versión de tienda. La prueba física posterior descubrió el fallo de reanudación descrito en F-041, por lo que queda sustituida para QA interno por 1.0.4 (1). App Review continúa bloqueado hasta completar QA físico y hasta que AdMob reconozca Developer Website y permita verificar cuenta, app e IDs reales activos. Si el código no cambia tras ese QA, se generará 1.0.4 (2) de producción, se repetirá el preflight contra ese binario exacto y se mantendrá publicación manual.

### D-046 - La intención explícita de grabar gobierna la recuperación de interrupciones - 2026-08-30

La recomendación `shouldResume` deja de ser una puerta para VoiceRecorder: Apple la define como indicación para la reanudación automática de reproducción y permite ignorarla cuando no se necesita una nueva acción del usuario. Mientras el usuario no pulse Stop, un fin de interrupción con o sin recomendación intenta reactivar inmediatamente; un retry único cada cinco segundos cubre activación todavía denegada y ausencia de notificación de fin. Foreground ofrece otra oportunidad. Stop cancela todos los caminos y un backend ya activo nunca se duplica. `setPrefersNoInterruptionsFromSystemAlerts(true)` se usa como preferencia durante la captura, no como promesa de impedir Siri, llamadas aceptadas u otras exclusividades del sistema. Esta decisión sustituye el criterio de `shouldResume` de D-041.
...
El commit `7df0dea` contiene la instrumentación interna. `33340707076` pasó XCTest y Release; `33340921074` pasó firma, archive, IPA, validación Apple y upload con IDs demo. El finalizador de solo lectura `33341464009` confirmó el recurso `53a1f112-27b7-4e31-be57-a2f5a130b73f`: build exacta 1.0.6 (1), `VALID`, `INTERNAL_ONLY`, `usesNonExemptEncryption=false`, `IN_BETA_TESTING`, externo `NOT_APPLICABLE`, grupo privado `Testers` (`9fb339bc-471a-438f-bc2e-f9961e974cee`) con dos testers, acceso automático a todas las builds, relación activa y `selectedByAppStoreVersions=[]`. No se seleccionó la build, no se editaron metadata/trains y no hubo TestFlight externo, App Review ni publicación.

### F-047 - El diagnóstico físico demuestra reanudación y escritura tras Siri - 2026-08-31

El JSON físico de iOS 16.7.16 demuestra que 1.0.6 conservó la intención, cerró el primer segmento al comenzar Siri y reanudó al regresar a foreground. Los cuatro intentos de background fallaron con `560557684` (`!int`), que Apple define como `AVAudioSession.ErrorCode.cannotInterruptOthers`; la primera activación válida coincidió con el retorno a la app. El propietario confirmó que, sin volver manualmente, el indicador naranja no reaparece y no existe captura. Por tanto, esta evidencia sustituye la interpretación inicial: 1.0.6 no satisface la continuidad automática en background y queda limitada a diagnóstico interno.

### D-052 - La sesión de grabación debe ser mezclable para recuperar desde background - 2026-08-31

`playAndRecord` es no mezclable por defecto y Apple documenta que una sesión no mezclable no puede activarse desde background si para ello debe interrumpir otra sesión. `mixWithOthers` es una opción válida para `playAndRecord` y elimina esa causa concreta. La candidata iOS 1.0.7 (1) usa `[allowBluetoothHFP, defaultToSpeaker, mixWithOthers]` en cada activación inicial y de recovery, conservando la reconstrucción, el retry y la cancelación por Stop. La contrapartida aceptada para QA es que otro audio puede continuar y ser captado acústicamente; no se usa `duckOthers` ni se interrumpe audio hablado. La candidata permanece local hasta validar en macOS y en iPhone que Siri termina, la app sigue sin abrirse y el micrófono vuelve por sí solo.
