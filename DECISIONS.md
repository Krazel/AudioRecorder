...
Toda versión pública que conserve ATT y permita a AdMob usar IDFA cuando el usuario autoriza debe declarar `usesIdfa=true` y completar en App Store Connect las finalidades IDFA coherentes antes de enviarse: sirve anuncios, respeta la elección ATT y no atribuye instalaciones ni acciones internas a anuncios previos. Estas declaraciones remotas forman parte del preflight obligatorio junto con `NSUserTrackingUsageDescription`, App Privacy y la prueba del flujo ATT. Si una versión futura elimina por completo el uso de IDFA, solo podrán cambiarse después de reconciliar código, SDKs, política y App Privacy.

### F-024 - `INVALID_BINARY` de build 6 corregido y reenviado - 2026-08-15

El primer envío conjunto de 1.0 (6) quedó en `INVALID_BINARY` porque `appStoreVersions.usesIdfa` era `null`, aunque el binario incluye ATT y Google Mobile Ads. Apple mantuvo la build 6 `VALID`, `APP_STORE_ELIGIBLE`, no caducada y sin diagnósticos de carga. Se corrigió el booleano a `true`; la versión volvió a `READY_FOR_REVIEW`, pero el reenvío de las 10:59:44 UTC regresó automáticamente a `INVALID_BINARY` porque aún faltaba completar las finalidades IDFA en la interfaz. El antiguo recurso detallado `idfaDeclarations` ya no existe en la API pública, por lo que el ítem se dejó otra vez resuelto/listo, sin un tercer reenvío, a la espera de completar esa única pantalla autenticada. No se generó una build 7 ni se alteró Android. La publicación sigue siendo manual.
