...
El primer envío conjunto de 1.0 (6) quedó en `INVALID_BINARY` porque `appStoreVersions.usesIdfa` era `null`, aunque el binario incluye ATT y Google Mobile Ads. Apple mantuvo la build 6 `VALID`, `APP_STORE_ELIGIBLE`, no caducada y sin diagnósticos de carga. Se corrigió el booleano a `true`; la versión volvió a `READY_FOR_REVIEW`. Un reenvío inmediato volvió a caer mientras la corrección aún se estaba asentando. Tras resolver de nuevo el ítem, verificar `usesIdfa=true` y confirmar en Chrome que no faltaba ningún campo vigente, la misma submission se reenvió a las 12:06:58 UTC. La API y la interfaz autenticada muestran los nueve elementos `WAITING_FOR_REVIEW`. No se generó una build 7 ni se alteró Android. La publicación sigue siendo manual.
