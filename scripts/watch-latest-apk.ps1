param(
  ...
  [string]$ArtifactName = "AudioRecorder-android-apk",
...
$headers = @{
  "User-Agent" = "AudioRecorder-Android-Artifact-Watcher"
...
for ($attempt = 1; $attempt -le $MaxAttempts; $attempt += 1) {
  ...
  if ($null -eq $run) {
    ...
    $runDir = Join-Path $oldDir "AudioRecorder-android-apk-run-$($run.id)"
    New-Item -ItemType Directory -Force -Path $runDir | Out-Null
    $versionedPath = Join-Path $artifactDir "AudioRecorder-Android-v$AppVersion-build-$($run.run_number).apk"
    $downloadPath = Join-Path $runDir "AudioRecorder-Android-latest.apk"
    $releaseUrl = "https://github.com/$Repo/releases/download/latest-android/AudioRecorder-Android-latest.apk"

    Move-CurrentArtifacts
    try {
      Invoke-WebRequest -Uri $releaseUrl -Headers @{ "User-Agent" = "AudioRecorder-Android-Artifact-Watcher" } -OutFile $downloadPath
      ...
      $zipPath = Join-Path $runDir "AudioRecorder-android-apk-run-$($run.id).zip"
      ...
      $apkPath = Join-Path $runDir "AudioRecorder-Android-latest.apk"
      if (!(Test-Path -LiteralPath $apkPath)) {
        throw "No se encontro AudioRecorder-Android-latest.apk dentro del artifact."
      }
